...
export function KBEditor({
    ...
    const getPreviewContent = useCallback(() => {
        ...
        return markdown
            ...
            .replace(/^## (.+)$/gm, '<h2 class="text-2xl font-semibold mb-3 mt-6 text-blue-400">$1</h2>')
            .replace(/^### (.+)$/gm, '<h3 class="text-xl font-medium mb-2 mt-4 text-gray-300">$1</h3>')
            .replace(/^#### (.+)$/gm, '<h4 class="text-lg font-medium mb-2 mt-3 text-gray-400">$1</h4>')
            .replace(/^\*\*(.+)\*\*:\s*(.+)$/gm, '<p class="mb-1"><strong class="text-gray-300">$1:</strong> $2</p>')
            ...
            .replace(/^\> (.+)$/gm, '<blockquote class="border-l-4 border-blue-500 pl-4 text-gray-400 italic">$1</blockquote>')
            .replace(/\n\n/g, '</p><p class="mb-2">')
            .replace(/\|([^\n]+)\|/g, (match) => {
                if (match.includes('---')) return '';
                const cells = match.split('|').filter(c => c.trim());
                return `<tr>${cells.map(c => `<td class="border border-gray-700 px-2 py-1">${c.trim()}</td>`).join('')}</tr>`;
            });
    }, [markdown]);
        ...
        <div className="flex flex-col h-full bg-gray-900 rounded-lg border border-gray-700">
            ...
            <div className="flex items-center justify-between p-3 border-b border-gray-700 bg-gray-800/50">
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-gray-200">{kbName}</span>
                    {hasChanges && (
                ...
                <div className="flex items-center gap-2">
                    ...
                    <label className="p-2 text-gray-400 hover:text-white hover:bg-gray-700 rounded transition cursor-pointer" title="Import Markdown">
            ...
            {/* Error Banner */}
            {errors.length > 0 && (
                <div className="bg-red-900/30 border-b border-red-700 p-3">
                            ...
                            <ul className="text-sm text-red-300 mt-1">
                                {errors.map((err, i) => (
                                    <li key={i}>
                                        ...
                                        {err.suggestion && <span className="text-red-400/70"> - {err.suggestion}</span>}
                                    </li>
                                ))}
                            </ul>
                        </div>
            ...
            {/* Status Bar */}
            <div className="flex items-center justify-between px-3 py-2 border-t border-gray-700 bg-gray-800/50 text-xs text-gray-500">
