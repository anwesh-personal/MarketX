...
type Tab = 'grid' | 'templates' | 'deploy' | 'logs';
...
export default function WorkerControlDashboard() {
    ...
    const tabs = [
        ...
        { id: 'deploy' as Tab, label: 'Deploy', icon: Settings },
    ...
    if (vpsServers.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center h-screen">
                <AlertCircle className="w-16 h-16 text-textTertiary mb-lg opacity-50" />
                <h2 className="text-2xl font-bold text-textPrimary mb-sm">No VPS Servers Configured</h2>
                <p className="text-textSecondary mb-lg">Add a VPS server to start managing workers</p>
                <button
                    onClick={() => {/* Navigate to VPS settings */ }}
    ...
    return (
        <div className="h-screen flex flex-col bg-background">
            ...
            <div className="flex-1 overflow-auto">
                ...
                {activeTab === 'deploy' && (
                    <DeploymentCreator serverId={selectedServer} />
                )}

                {activeTab === 'logs' && (
                    <LogsViewer serverId={selectedServer} />
                )}
            </div>
        </div>
    );
