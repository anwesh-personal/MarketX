...
import { enqueueAllocationRebalance } from '@/lib/queues'
...
export async function POST(req: NextRequest) {
    ...
    if (!['owner', 'admin', 'superadmin'].includes(me.role ?? '')) {
        return NextResponse.json({ error: 'Only admin/owner can enqueue allocation rebalance' }, { status: 403 })
    }
    ...
    const queued = await enqueueAllocationRebalance({
        orgId: targetOrgId,
        lookbackDays,
    })
    ...
    return NextResponse.json({
        success: true,
        message: 'Allocation rebalance job queued',
