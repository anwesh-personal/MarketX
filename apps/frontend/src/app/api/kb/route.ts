...
export async function POST(request: NextRequest) {
    try {
        ...
        const defaultKB = {
            schema_version: '1.0.0', kb_version: '1.0.0', stage: 'pre-embeddings',
            brand: { brand_name_exact: '', voice_rules: [], compliance: { forbidden_claims: [], required_disclosures: [] } },
            icp_library: { segments: [] }, offer_library: { offers: [] }, angles_library: { angles: [] },
            ctas_library: { ctas: [] }, website_library: { page_blueprints: [], layouts: [] },
            email_library: { flow_blueprints: [], subject_firstline_variants: [], reply_playbooks: [], reply_strategies: [] },
            social_library: { pillars: [], post_blueprints: [] }, routing: { defaults: [], rules: [] },
            testing: {
            ...
            guardrails: { paused_patterns: [] }, learning: { history: [], preferences: [] },
        }

        const { data, error } = await supabaseAdmin
            .from('knowledge_bases')
            .insert({ name, org_id: ctx.orgId, data: kbData || defaultKB, version: 1, created_by: ctx.userId })
            .select()
            .single()
