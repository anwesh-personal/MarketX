    ...
    try {
        switch (nodeType) {
            ...
            case 'ai': {
                ...
                // Call AI service — use org-aware BYOK chain when orgId is available
                ...
                const aiResult: AICallResult = orgId
                    ? await aiService.callWithOrgContext(orgId, resolvedPrompt, aiCallOptions)
                    : await aiService.call(resolvedPrompt, aiCallOptions);
            ...
            default: {
                ...
                if (nodeType.startsWith('agent-')) {
                    ...
                    // 2. Load assigned prompt_blocks for THIS org_agent only.
                    //    Brain-level prompt assembly was already done at queue time
                    //    (via PromptStackAssembler) and is available in pipelineData.userInput.brain_context.prompt_stack
                    ...
                    const hasAgentBlocks = assignedPrompts && assignedPrompts.length > 0;

                    const systemPromptParts: string[] = [];

                    // Use pre-assembled brain prompt stack from input (if available)
                    const preAssembled = pipelineData.userInput?.brain_context?.prompt_stack;
                    if (preAssembled) {
                        if (preAssembled.foundation) systemPromptParts.push(`### BRAIN FOUNDATION:\n${preAssembled.foundation}`);
                        if (preAssembled.persona) systemPromptParts.push(`### BRAIN PERSONA:\n${preAssembled.persona}`);
                        if (preAssembled.domain) systemPromptParts.push(`### BRAIN DOMAIN:\n${preAssembled.domain}`);
                        if (preAssembled.instructions) systemPromptParts.push(`### BRAIN INSTRUCTIONS:\n${preAssembled.instructions}`);
                        console.log(`    📝 Using pre-assembled brain prompt stack from input`);
                    } else {
                        // Fallback: inline prompts from brain_agent row (no pre-assembly available)
                        ...
                        console.log(`    📝 Using inline brain prompts (no pre-assembled stack)`);
                    }

                    // Append org_agent-specific prompts (inline)
                    ...
                    // Append org_agent prompt blocks (per-agent, not brain-level)
                    if (hasAgentBlocks) {
                        const categoryOrder = ['foundation', 'persona', 'domain', 'instruction', 'guardrails', 'task', 'custom'];
                        for (const cat of categoryOrder) {
                            const blocks = (assignedPrompts ?? []).filter((a: any) => (a as any).prompt_blocks?.category === cat);
                            for (const a of blocks) {
                                const block = (a as any).prompt_blocks;
                                if (block?.content) {
                                    systemPromptParts.push(`### AGENT ${cat.toUpperCase()}:\n${block.content}`);
                                }
                            }
                        }
                        console.log(`    📝 Loaded ${assignedPrompts!.length} agent-specific prompt blocks`);
                    }

                    // Guardrails (brain + agent)
                    if (preAssembled?.guardrails) {
                        systemPromptParts.push(`### GUARDRAILS:\n${preAssembled.guardrails}`);
                    } else if (brainAgent?.guardrails_prompt) {
