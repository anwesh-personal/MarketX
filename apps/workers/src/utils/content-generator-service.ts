/**
 * CONTENT GENERATOR SERVICE (WORKERS VERSION)
 * 
 * Unified service for generating all content types:
 * - Website Pages / Bundles
 * - Email Flows
 * - Email Replies
 * - Social Posts
 * 
 * PORTED FROM: apps/backend/src/services/content/contentGeneratorService.ts
 * 
 * PRINCIPLES:
...
export interface PageOutput {
    ...
    routing_suggestions: Array<{ next_page_slug: string; condition: string }>;
    generated_at: string;
}
...
class ContentGeneratorService {
    ...
    async generateWebsiteBundle(input: BundleGeneratorInput): Promise<WebsiteBundle> {
        ...
        const routingMap: WebsiteBundle['routing_map'] = [];
    ...
    async generateEmailFlow(input: EmailFlowGeneratorInput): Promise<EmailFlowBundle> {
        const { kb, icp, offer, angle, flowBlueprint, emailCount } = input;
        const generatedAt = new Date().toISOString();
    ...
    async generateEmailReply(input: EmailReplyGeneratorInput): Promise<EmailReplyBundle> {
        const { kb, incomingEmail, detectedScenario, strategy } = input;
        const generatedAt = new Date().toISOString();
    ...
    async generateSocialPost(input: SocialPostGeneratorInput): Promise<SocialPostBundle> {
        const { kb, icp, offer, angle, platform, postBlueprint, pillar } = input;
        const generatedAt = new Date().toISOString();
