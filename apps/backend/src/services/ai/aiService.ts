/**
 * AXIOM AI SERVICE
 * Full port from Lekhika's aiService.js
 * 
...
export interface EngineApiKeyContext {
    engineId: string;
    orgId?: string;
    apiKeyOverride?: string; // Direct key override
...
export interface AICallOptions {
    ...
    /** Engine context for API key isolation */
...
class AIService {
    ...
    /**
     * Initialize with database pool and load API keys
     */
    ...
    private async loadApiKeys(): Promise<void> {
        if (!this.dbPool) {
            console.warn('⚠️ AI Service: No database pool, using environment variables');
    ...
    async call(prompt: string, options: AICallOptions = {}): Promise<AICallResult> {
        ...
        if (options.engineContext) {
            // Engine-level API key isolation
