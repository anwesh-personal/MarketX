 ...
 * 
 * SCHEMA REFERENCE: apps/backend/src/schemas/kb.schema.ts
 */
...
class KBResolutionService {
    ...
    // Note: OfferSchema does NOT have applies_to, so we match by category/name
    // =========================================================================

    resolveOffer(
        context: ResolutionContext,
        hints: {
            offerId?: string; // Direct specification
            categoryHint?: string;
    ...
    selectAngle(
        ...
        params: {
            ...
            offerId?: string;
            buyerStage?: string;
            preferredAxis?: string; // Force a specific axis
        }
    ): ResolvedAngle {
        const { kb } = context;
        const angles = kb.angles_library.angles;

        // Check KB learning preferences first (from learning loop)
        // Schema: KBLearningPreferenceSchema has preferred_ids (array), not value_id
        ...
        const anglePreference = preferences.find((p: KBLearningPreference) =>
            ...
            // applies_to uses singular icp_id, not arrays
        ...
        if (anglePreference && anglePreference.preferred_ids.length > 0) {
            // preferred_ids is an array - use first preferred
        ...
        // AngleSchema.applies_to uses singular icp_id
        const applicableAngles = angles.filter((a: Angle) =>
            !a.applies_to?.icp_id || a.applies_to.icp_id === params.icpId
        );

        // Select based on buyer stage mapping (if available)
        // Mapping buyer stages to recommended angle axes
    ...
    selectBlueprint(
        ...
        params: {
            ...
            pageType?: string; // For website: LANDING, HOW_IT_WORKS, etc.
        ...
        switch (params.contentType) {
            case 'website_page': {
                ...
                // LayoutSchema uses applies_to (with buyer_stage), not page_types
            ...
            case 'email_reply': {
                ...
                const playbook = playbooks[0]; // Default to first playbook
    ...
    // Select reply strategy for email reply generation
    // =========================================================================
    ...
    selectCTA(
        ...
        // Check routing rules
        // RoutingRuleSchema.if has: entry_page_type, buyer_stage, icp_id
    ...
    // What pages should come next based on current context
    // =========================================================================
