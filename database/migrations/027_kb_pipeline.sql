-- ============================================================
-- MIGRATION 027: KB PIPELINE
-- Structured knowledge base with named sections and per-section
-- access control. Documents go through an async processing pipeline.
--
-- lock_level controls who can edit each section:
--   superadmin  → only superadmin can edit (e.g. core guardrails)
--   org_admin   → org admin can edit (e.g. brand voice, offer)
--   user        → any member can edit (e.g. personal examples)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS kb_sections (
  ...
  -- Identity
  name          TEXT  NOT NULL,          -- machine key: 'brand_voice', 'icp', 'offer', 'angles', 'guardrails', 'examples'
  ...
  -- Stats (denormalized — updated by trigger on kb_documents)
...
CREATE TABLE IF NOT EXISTS kb_documents (
  ...
  content_hash   TEXT  NOT NULL,          -- SHA-256: detect changes for re-embedding
  file_name      TEXT,
  file_type      TEXT,                    -- 'pdf', 'docx', 'txt', 'md', 'manual'

  -- Processing pipeline state
  status         TEXT  NOT NULL DEFAULT 'pending'
                   CHECK (status IN ('pending', 'chunking', 'embedding', 'ready', 'error', 'stale')),
  chunk_count    INT   NOT NULL DEFAULT 0,
...
-- ============================================================
-- TRIGGER: keep kb_sections.doc_count + chunk_count in sync
-- ============================================================
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'kb_sections') THEN
    RAISE EXCEPTION 'Migration 027 FAILED: kb_sections not created';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'kb_documents') THEN
    RAISE EXCEPTION 'Migration 027 FAILED: kb_documents not created';
  END IF;
  RAISE NOTICE '✓ Migration 027: kb_sections + kb_documents created';
END $$;
